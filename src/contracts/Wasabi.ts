...
export const deployAddress: Address | undefined = '0xe67Ef17B6c82a555CB040173273FB271fcc43BEd'
...
export type Contract = {
  calls: {
    ...
    WASABI_LONG_POOL: () => Promise<`0x${string}`>
    WASABI_SHORT_POOL: () => Promise<`0x${string}`>
...
export const call: CallType = {
  ...
  WASABI_LONG_POOL: (...args: ExtractArgs<Contract['calls']['WASABI_LONG_POOL']>) =>
    getRequest('WASABI_LONG_POOL', args),
  WASABI_SHORT_POOL: (...args: ExtractArgs<Contract['calls']['WASABI_SHORT_POOL']>) =>
    getRequest('WASABI_SHORT_POOL', args),
...
export type SDK = {
  ...
  abi: typeof abi
  events: {
    WasabiDeposit: { topic: Address; parse: (events: (RpcLog | Log)[]) => ParsedEvent<'WasabiDeposit'>[] }
    WasabiWithdrawal: { topic: Address; parse: (events: (RpcLog | Log)[]) => ParsedEvent<'WasabiWithdrawal'>[] }
  ...
  WASABI_LONG_POOL: (
    ...args: ExtractArgs<Contract['calls']['WASABI_LONG_POOL']>
  ) => Promise<CallReturn<'WASABI_LONG_POOL'>>
  WASABI_SHORT_POOL: (
    ...args: ExtractArgs<Contract['calls']['WASABI_SHORT_POOL']>
  ) => Promise<CallReturn<'WASABI_SHORT_POOL'>>
...
export function toSdk(
  ...
  return {
    ...
    events: {
      WasabiDeposit: {
        topic: getEventTopic('WasabiDeposit'),
        parse: (events: (RpcLog | Log)[]) => parseEvents('WasabiDeposit', events),
      },
      WasabiWithdrawal: {
        topic: getEventTopic('WasabiWithdrawal'),
        parse: (events: (RpcLog | Log)[]) => parseEvents('WasabiWithdrawal', events),
      },
      LegoPauseModified: {
    ...
    hasClaimableRewards: (...args: ExtractArgs<Contract['calls']['hasClaimableRewards']>) =>
      ...
      >,
    WASABI_LONG_POOL: (...args: ExtractArgs<Contract['calls']['WASABI_LONG_POOL']>) =>
      singleQuery(publicClient!, call.WASABI_LONG_POOL(...args), {}, addressResolver) as Promise<
        CallReturn<'WASABI_LONG_POOL'>
      >,
    WASABI_SHORT_POOL: (...args: ExtractArgs<Contract['calls']['WASABI_SHORT_POOL']>) =>
      singleQuery(publicClient!, call.WASABI_SHORT_POOL(...args), {}, addressResolver) as Promise<
        CallReturn<'WASABI_SHORT_POOL'>
      >,
