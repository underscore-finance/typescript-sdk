...
export type SDK = {
  ...
  abi: typeof abi
  events: {
    FluidDeposit: { topic: Address; parse: (events: (RpcLog | Log)[]) => ParsedEvent<'FluidDeposit'>[] }
    FluidWithdrawal: { topic: Address; parse: (events: (RpcLog | Log)[]) => ParsedEvent<'FluidWithdrawal'>[] }
...
export function toSdk(
  ...
  return {
    ...
    events: {
      FluidDeposit: {
        topic: getEventTopic('FluidDeposit'),
        parse: (events: (RpcLog | Log)[]) => parseEvents('FluidDeposit', events),
      },
      FluidWithdrawal: {
        topic: getEventTopic('FluidWithdrawal'),
        parse: (events: (RpcLog | Log)[]) => parseEvents('FluidWithdrawal', events),
      },
      LegoPauseModified: {
    ...
    FLUID_RESOLVER: (...args: ExtractArgs<Contract['calls']['FLUID_RESOLVER']>) =>
      singleQuery(publicClient!, call.FLUID_RESOLVER(...args), {}, addressResolver) as Promise<
        CallReturn<'FLUID_RESOLVER'>
      >,
    ...
    NATIVE_ETH: (...args: ExtractArgs<Contract['calls']['NATIVE_ETH']>) =>
      singleQuery(publicClient!, call.NATIVE_ETH(...args), {}, addressResolver) as Promise<CallReturn<'NATIVE_ETH'>>,
