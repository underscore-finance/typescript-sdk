...
export type SDK = {
  ...
  abi: typeof abi
  events: {
    AaveV3Deposit: { topic: Address; parse: (events: (RpcLog | Log)[]) => ParsedEvent<'AaveV3Deposit'>[] }
    AaveV3Withdrawal: { topic: Address; parse: (events: (RpcLog | Log)[]) => ParsedEvent<'AaveV3Withdrawal'>[] }
...
export function toSdk(
  ...
  return {
    ...
    events: {
      AaveV3Deposit: {
        topic: getEventTopic('AaveV3Deposit'),
        parse: (events: (RpcLog | Log)[]) => parseEvents('AaveV3Deposit', events),
      },
      AaveV3Withdrawal: {
        topic: getEventTopic('AaveV3Withdrawal'),
        parse: (events: (RpcLog | Log)[]) => parseEvents('AaveV3Withdrawal', events),
      },
      LegoPauseModified: {
    ...
    AAVE_V3_POOL: (...args: ExtractArgs<Contract['calls']['AAVE_V3_POOL']>) =>
      singleQuery(publicClient!, call.AAVE_V3_POOL(...args), {}, addressResolver) as Promise<
        CallReturn<'AAVE_V3_POOL'>
      >,
    AAVE_V3_ADDRESS_PROVIDER: (...args: ExtractArgs<Contract['calls']['AAVE_V3_ADDRESS_PROVIDER']>) =>
      singleQuery(publicClient!, call.AAVE_V3_ADDRESS_PROVIDER(...args), {}, addressResolver) as Promise<
