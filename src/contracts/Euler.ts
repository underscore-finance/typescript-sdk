...
export const deployAddress: Address | undefined = '0x7f52A8bCF7589e157961dbc072e1C5E45A3F4fd6'
...
export const call: CallType = {
  ...
  EULER_EARN_FACTORY: (...args: ExtractArgs<Contract['calls']['EULER_EARN_FACTORY']>) =>
    getRequest('EULER_EARN_FACTORY', args),
...
export type SDK = {
  ...
  abi: typeof abi
  events: {
    EulerDeposit: { topic: Address; parse: (events: (RpcLog | Log)[]) => ParsedEvent<'EulerDeposit'>[] }
    EulerWithdrawal: { topic: Address; parse: (events: (RpcLog | Log)[]) => ParsedEvent<'EulerWithdrawal'>[] }
  ...
  ) => Promise<CallReturn<'EULER_EVAULT_FACTORY'>>
  EULER_EARN_FACTORY: (
    ...args: ExtractArgs<Contract['calls']['EULER_EARN_FACTORY']>
  ) => Promise<CallReturn<'EULER_EARN_FACTORY'>>
...
export function toSdk(
  ...
  return {
    ...
    events: {
      EulerDeposit: {
        topic: getEventTopic('EulerDeposit'),
        parse: (events: (RpcLog | Log)[]) => parseEvents('EulerDeposit', events),
      },
      EulerWithdrawal: {
        topic: getEventTopic('EulerWithdrawal'),
        parse: (events: (RpcLog | Log)[]) => parseEvents('EulerWithdrawal', events),
      },
      EulerRewardsAddrSet: {
    ...
    eulerRewards: (...args: ExtractArgs<Contract['calls']['eulerRewards']>) =>
      singleQuery(publicClient!, call.eulerRewards(...args), {}, addressResolver) as Promise<
        CallReturn<'eulerRewards'>
      >,
    EULER_EVAULT_FACTORY: (...args: ExtractArgs<Contract['calls']['EULER_EVAULT_FACTORY']>) =>
      singleQuery(publicClient!, call.EULER_EVAULT_FACTORY(...args), {}, addressResolver) as Promise<
        CallReturn<'EULER_EVAULT_FACTORY'>
      >,
    EULER_EARN_FACTORY: (...args: ExtractArgs<Contract['calls']['EULER_EARN_FACTORY']>) =>
      singleQuery(publicClient!, call.EULER_EARN_FACTORY(...args), {}, addressResolver) as Promise<
        CallReturn<'EULER_EARN_FACTORY'>
      >,
