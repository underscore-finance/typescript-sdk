...
export const deployAddress: Address | undefined = '0x590FB39919c5F0323a93B54f2634d010b6ECfbA7'
...
function getRequest<M extends Calls>(
  ...
  const call = {
    contractName: 'CompoundV3' as const,
    method,
    args,
    address,
    deployAddress,
...
export type SDK = {
  ...
  abi: typeof abi
  events: {
    CompoundV3Deposit: { topic: Address; parse: (events: (RpcLog | Log)[]) => ParsedEvent<'CompoundV3Deposit'>[] }
    CompoundV3Withdrawal: { topic: Address; parse: (events: (RpcLog | Log)[]) => ParsedEvent<'CompoundV3Withdrawal'>[] }
    CompoundV3RewardsAddrSet: {
      topic: Address
      parse: (events: (RpcLog | Log)[]) => ParsedEvent<'CompoundV3RewardsAddrSet'>[]
    }
...
export function toSdk(
  ...
  return {
    ...
    events: {
      CompoundV3Deposit: {
        topic: getEventTopic('CompoundV3Deposit'),
        parse: (events: (RpcLog | Log)[]) => parseEvents('CompoundV3Deposit', events),
      },
      CompoundV3Withdrawal: {
        topic: getEventTopic('CompoundV3Withdrawal'),
        parse: (events: (RpcLog | Log)[]) => parseEvents('CompoundV3Withdrawal', events),
      },
      CompoundV3RewardsAddrSet: {
        topic: getEventTopic('CompoundV3RewardsAddrSet'),
        parse: (events: (RpcLog | Log)[]) => parseEvents('CompoundV3RewardsAddrSet', events),
      },
      LegoPauseModified: {
    ...
    compoundRewards: (...args: ExtractArgs<Contract['calls']['compoundRewards']>) =>
      singleQuery(publicClient!, call.compoundRewards(...args), {}, addressResolver) as Promise<
        CallReturn<'compoundRewards'>
      >,
    COMPOUND_V3_CONFIGURATOR: (...args: ExtractArgs<Contract['calls']['COMPOUND_V3_CONFIGURATOR']>) =>
      singleQuery(publicClient!, call.COMPOUND_V3_CONFIGURATOR(...args), {}, addressResolver) as Promise<
    ...
    hasClaimableRewards: (...args: ExtractArgs<Contract['mutations']['hasClaimableRewards']>) =>
      mutate(walletClient!, mutation.hasClaimableRewards, { addressResolver })(...args),
