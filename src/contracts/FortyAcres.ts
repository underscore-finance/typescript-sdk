...
export const deployAddress: Address | undefined = '0x39F5EDd73ce1682Da63C92C34fBbBEdB07156514'
...
export type Contract = {
  calls: {
    ...
    FORTY_ACRES_LOANS: () => Promise<`0x${string}`>
  }
  mutations: {
...
function getRequest<M extends Calls>(
  ...
  const call = {
    contractName: 'FortyAcres' as const,
    method,
    args,
    address,
    deployAddress,
...
export const call: CallType = {
  ...
  FORTY_ACRES_USDC_VAULT: (...args: ExtractArgs<Contract['calls']['FORTY_ACRES_USDC_VAULT']>) =>
    getRequest('FORTY_ACRES_USDC_VAULT', args),
  FORTY_ACRES_LOANS: (...args: ExtractArgs<Contract['calls']['FORTY_ACRES_LOANS']>) =>
    getRequest('FORTY_ACRES_LOANS', args),
}
...
export type SDK = {
  ...
  abi: typeof abi
  events: {
    FortyAcresDeposit: { topic: Address; parse: (events: (RpcLog | Log)[]) => ParsedEvent<'FortyAcresDeposit'>[] }
    FortyAcresWithdrawal: { topic: Address; parse: (events: (RpcLog | Log)[]) => ParsedEvent<'FortyAcresWithdrawal'>[] }
  ...
  FORTY_ACRES_USDC_VAULT: (
    ...args: ExtractArgs<Contract['calls']['FORTY_ACRES_USDC_VAULT']>
  ) => Promise<CallReturn<'FORTY_ACRES_USDC_VAULT'>>
  FORTY_ACRES_LOANS: (
    ...args: ExtractArgs<Contract['calls']['FORTY_ACRES_LOANS']>
  ) => Promise<CallReturn<'FORTY_ACRES_LOANS'>>
  ...
  return {
    ...
    events: {
      FortyAcresDeposit: {
        ...
        parse: (events: (RpcLog | Log)[]) => parseEvents('FortyAcresDeposit', events),
      },
      FortyAcresWithdrawal: {
        topic: getEventTopic('FortyAcresWithdrawal'),
        parse: (events: (RpcLog | Log)[]) => parseEvents('FortyAcresWithdrawal', events),
      },
      LegoPauseModified: {
    ...
    RIPE_REGISTRY: (...args: ExtractArgs<Contract['calls']['RIPE_REGISTRY']>) =>
      ...
      >,
    FORTY_ACRES_USDC_VAULT: (...args: ExtractArgs<Contract['calls']['FORTY_ACRES_USDC_VAULT']>) =>
      singleQuery(publicClient!, call.FORTY_ACRES_USDC_VAULT(...args), {}, addressResolver) as Promise<
        CallReturn<'FORTY_ACRES_USDC_VAULT'>
      >,
    FORTY_ACRES_LOANS: (...args: ExtractArgs<Contract['calls']['FORTY_ACRES_LOANS']>) =>
      singleQuery(publicClient!, call.FORTY_ACRES_LOANS(...args), {}, addressResolver) as Promise<
        CallReturn<'FORTY_ACRES_LOANS'>
      >,
