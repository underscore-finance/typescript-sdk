...
export const deployAddress: Address | undefined = '0xc88CD884bdFa8F2D93e32a13EE16543b8a2CF1f0'
...
export type Contract = {
  calls: {
    ...
    AVANTIS_USDC_VAULT: () => Promise<`0x${string}`>
...
export const call: CallType = {
  ...
  AVANTIS_USDC_VAULT: (...args: ExtractArgs<Contract['calls']['AVANTIS_USDC_VAULT']>) =>
    getRequest('AVANTIS_USDC_VAULT', args),
...
export type SDK = {
  ...
  abi: typeof abi
  events: {
    AvantisDeposit: { topic: Address; parse: (events: (RpcLog | Log)[]) => ParsedEvent<'AvantisDeposit'>[] }
    AvantisWithdrawal: { topic: Address; parse: (events: (RpcLog | Log)[]) => ParsedEvent<'AvantisWithdrawal'>[] }
  ...
  AVANTIS_USDC_VAULT: (
    ...args: ExtractArgs<Contract['calls']['AVANTIS_USDC_VAULT']>
  ) => Promise<CallReturn<'AVANTIS_USDC_VAULT'>>
...
export function toSdk(
  ...
  return {
    ...
    events: {
      AvantisDeposit: {
        topic: getEventTopic('AvantisDeposit'),
        parse: (events: (RpcLog | Log)[]) => parseEvents('AvantisDeposit', events),
      },
      AvantisWithdrawal: {
        topic: getEventTopic('AvantisWithdrawal'),
        parse: (events: (RpcLog | Log)[]) => parseEvents('AvantisWithdrawal', events),
      },
      LegoPauseModified: {
    ...
    hasClaimableRewards: (...args: ExtractArgs<Contract['calls']['hasClaimableRewards']>) =>
      ...
      >,
    AVANTIS_USDC_VAULT: (...args: ExtractArgs<Contract['calls']['AVANTIS_USDC_VAULT']>) =>
      singleQuery(publicClient!, call.AVANTIS_USDC_VAULT(...args), {}, addressResolver) as Promise<
        CallReturn<'AVANTIS_USDC_VAULT'>
      >,
