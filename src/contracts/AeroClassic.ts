...
export const deployAddress: Address | undefined = '0x43B2a72595016D765E2A66e4c2Cf3026619784D1'
...
export type SDK = {
  ...
  abi: typeof abi
  events: {
    AerodromeSwap: { topic: Address; parse: (events: (RpcLog | Log)[]) => ParsedEvent<'AerodromeSwap'>[] }
    AerodromeLiquidityAdded: {
      topic: Address
      parse: (events: (RpcLog | Log)[]) => ParsedEvent<'AerodromeLiquidityAdded'>[]
    }
    AerodromeLiquidityRemoved: {
      topic: Address
      parse: (events: (RpcLog | Log)[]) => ParsedEvent<'AerodromeLiquidityRemoved'>[]
    }
...
export function toSdk(
  ...
  return {
    ...
    events: {
      AerodromeSwap: {
        topic: getEventTopic('AerodromeSwap'),
        parse: (events: (RpcLog | Log)[]) => parseEvents('AerodromeSwap', events),
      },
      AerodromeLiquidityAdded: {
        topic: getEventTopic('AerodromeLiquidityAdded'),
        parse: (events: (RpcLog | Log)[]) => parseEvents('AerodromeLiquidityAdded', events),
      },
      AerodromeLiquidityRemoved: {
        topic: getEventTopic('AerodromeLiquidityRemoved'),
        parse: (events: (RpcLog | Log)[]) => parseEvents('AerodromeLiquidityRemoved', events),
      },
      LegoPauseModified: {
    ...
    getAccessForLego: (...args: ExtractArgs<Contract['calls']['getAccessForLego']>) =>
      ...
      >,
    AERODROME_FACTORY: (...args: ExtractArgs<Contract['calls']['AERODROME_FACTORY']>) =>
      singleQuery(publicClient!, call.AERODROME_FACTORY(...args), {}, addressResolver) as Promise<
        CallReturn<'AERODROME_FACTORY'>
      >,
    AERODROME_ROUTER: (...args: ExtractArgs<Contract['calls']['AERODROME_ROUTER']>) =>
      singleQuery(publicClient!, call.AERODROME_ROUTER(...args), {}, addressResolver) as Promise<
        CallReturn<'AERODROME_ROUTER'>
      >,
